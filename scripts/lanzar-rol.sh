...
# El rol tiene que ser un slug seguro. Este check es lo que impide que
# ROL="../../etc/passwd" construya paths fuera del contrato: el rol se usa
# para armar .claude/agents/<rol>.md y /etc/fabrica/tokens/<rol>.token.
# El regex prohibe '.', '/', mayusculas y todo lo que no sea [a-z0-9-], y
# ancla con ^$ — el traversal sigue siendo imposible.
#
# Issue #79: el ROSTER no se declara aca. Lo declara el conjunto de archivos
# de .claude/agents/ de cada repo, y quien lo hace cumplir es el chequeo de
# existencia de abajo (exit 3). Antes los siete nombres estaban hardcodeados,
# asi que agregar un rol exigia editar este script — y como el script viaja
# en la allowlist del sync, un parche en un repo producto lo revertia la
# proxima sincronizacion. La fabrica de videojuegos, que suma diseno-juego,
# gameplay y playtest, no podia lanzar ninguno de los tres.
#
# TODOS los roles pasan por este lanzador — revisores y constructores — porque
# el lanzador es lo que hace que un rol sea un TRABAJADOR de verdad: prompt de
# rol, identidad que corresponda, y atribucion de su trabajo en la bitacora.
if [[ ! "$ROL" =~ ^[a-z][a-z0-9-]{1,30}$ ]]; then
  echo "lanzar-rol: rol invalido: '$ROL'" >&2
  echo "lanzar-rol: esperado un slug de 2 a 31 caracteres: [a-z] seguido de [a-z0-9-]." >&2
  exit 2
fi

# Defensa en profundidad y, desde el issue #79, la definicion del roster: un
# slug bien formado que no tenga archivo de rol en ESTE repo no se lanza.
ROL_FILE=".claude/agents/${ROL}.md"
if [[ ! -f "$ROL_FILE" ]]; then
  echo "lanzar-rol: no existe $ROL_FILE" >&2
  echo "lanzar-rol: los roles de este repo son los archivos de .claude/agents/." >&2
