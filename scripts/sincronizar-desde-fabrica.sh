...
copiar_archivo() {
  ...
  # Issue #90: NUNCA encadenar los dos dialectos con `||` — en GNU, `stat -f`
  # significa "filesystem status" (no toma format string): imprime basura a
  # stdout con exit no-cero, y el `||` concatena el fallback ENCIMA de esa
  # basura. Probe explicito del dialecto contra /dev/null en su lugar.
  local src_mode
  if stat -c '%a' /dev/null >/dev/null 2>&1; then
    src_mode="$(stat -c '%a' "$src" 2>/dev/null || true)"   # GNU coreutils
  else
    src_mode="$(stat -f '%Lp' "$src" 2>/dev/null || true)"  # BSD/macOS
  fi
  # La guarda valida forma octal, no solo no-vacio: si algun dia stat vuelve
  # a emitir basura, el chmod no la recibe.
  if [[ ! "$src_mode" =~ ^[0-7]{3,4}$ ]] || ! chmod "$src_mode" "$tmp"; then
    rm -f "$tmp"
