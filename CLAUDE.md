...
- **Todo hallazgo nace como issue, ANTES de trabajar sobre el.** Una conversacion, chat o sesion de asistente no es registro: si no esta en GitHub, el hallazgo no existe. Aplica especialmente a hallazgos que no producen codigo (tareas de operador, config de servers, decisiones pendientes) — esos jamas pasan por un PR, asi que sin issue desaparecen sin dejar rastro. Para qa: trabajo en un PR que nacio sin issue previo es violacion de proceso. Secuencia con el circuito de revision: un hallazgo detectado DURANTE una revision se registra en el comentario firmado del PR (ese comentario es el registro del descubrimiento); los medios/bajos ademas nacen como issue en el mismo reporte, y el fix futuro nace de ese issue — la regla exige el issue antes del trabajo de CORRECCION, no antes de la revision que lo descubrio. Origen: issue fabrica#64 — ocho hallazgos de una sesion quedaron solo en el chat; los detecto el operador, no el proceso.
- **Excepcion — hallazgos de seguridad: NUNCA como issue publico antes del fix.** Se reportan como borrador de GitHub Security Advisory (Security → Advisories → New draft) en el mismo repo, que es privado por diseño; si el advisory no esta disponible, por canal privado al operador. Recien con el fix mergeado y desplegado se convierte (opcionalmente) en issue publico. Rige igual en repos publicos y privados, para que nadie dude que canal usar. Publicar el vector antes del parche es regalarselo a un atacante. Origen: revision de seguridad del PR fabrica#65.
- **Un issue jamas contiene secretos**: ni tokens, ni credenciales, ni valores literales de variables sensibles, ni PII de usuarios, ni contenido de archivos ignorados por git. Lo sensible se referencia por descripcion y ruta, nunca por valor (la edicion posterior NO borra lo que ya salio por mails, webhooks e indices externos). Un issue creado con un secreto es un incidente en si mismo: se revoca el secreto expuesto, se cierra ese issue y se abre otro sin el valor.
